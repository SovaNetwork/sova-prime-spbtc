...
contract RoleManagerTest is Test {
    ...
    function setUp() public {
        ...
        roleManager.grantRole(strategyManager, roleManager.STRATEGY_MANAGER());
        vm.stopPrank();
    }
    ...
    function test_ProtocolAdminCanGrantAnyRole() public {
        ...
        assertEq(roleManager.hasRole(user, roleManager.STRATEGY_ADMIN()), true);
        vm.stopPrank();
    }
    ...
    function test_ProtocolAdminCanRevokeAnyRole() public {
        ...
        assertEq(roleManager.hasRole(kycOperator, roleManager.KYC_OPERATOR()), false);
        vm.stopPrank();
    }
    ...
    function test_UserCanRenounceOwnRole() public {
        vm.startPrank(kycOperator);
        roleManager.renounceRole(roleManager.KYC_OPERATOR());
        assertEq(roleManager.hasRole(kycOperator, roleManager.KYC_OPERATOR()), false);
        vm.stopPrank();
    }
    ...
    function test_BatchRoleChecking() public {
        ...
        roles[1] = roleManager.KYC_ADMIN();
        
        assertEq(roleManager.hasAnyOfRoles(strategyAdmin, roles), true);
        assertEq(roleManager.hasAnyOfRoles(kycAdmin, roles), true);
        assertEq(roleManager.hasAnyOfRoles(user, roles), false);
        
        // Test hasAllRoles
        uint256[] memory rolesForAdmin = new uint256[](1);
        rolesForAdmin[0] = roleManager.PROTOCOL_ADMIN();
