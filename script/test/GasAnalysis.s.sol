...
contract GasAnalysisScript is Script {
    ...
    function reportGasMetrics(GasMetrics memory metrics) internal pure {
        ...
        // Calculate averages
        uint256 avgUserOp = (
            metrics.firstDeposit + metrics.subsequentDeposit + metrics.withdrawalRequest + metrics.withdrawalCompletion
        ) / 4;
        uint256 avgAdminOp = (
            metrics.withdrawalApproval + metrics.addLiquidity + metrics.removeLiquidity + metrics.addCollateral
                + metrics.removeCollateral + metrics.updateNav
        ) / 6;
        ...
        returns (OptimizationOpportunity[] memory)
    {
        ...
        uint256 totalWithdrawalGas =
            metrics.withdrawalRequest + metrics.withdrawalApproval + metrics.withdrawalCompletion;
    ...
    function generateComparativeAnalysis(GasMetrics memory metrics) internal pure {
        ...
        if (metrics.firstDeposit < aaveDeposit) {
            console2.log(
                "  Status: EFFICIENT (", ((aaveDeposit - metrics.firstDeposit) * 100) / aaveDeposit, "% better)"
            );
        } else {
        ...
        if (avgBtcVault < avgIndustry) {
            console2.log(
                "  BTC Vault is",
                ((avgIndustry - avgBtcVault) * 100) / avgIndustry,
    ...
    function generateMainnetCostEstimates(GasMetrics memory metrics) internal pure {
        ...
        uint256 dailyGas = (metrics.subsequentDeposit * 100) + (totalWithdrawal * 50)
            + (metrics.withdrawalApproval * 50) + metrics.updateNav;
