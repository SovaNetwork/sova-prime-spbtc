...
contract LoadTestScript is Script {
    ...
    function executeLoadTest(uint256 userCount, uint256 depositsPerUser, uint256 intervalMs)
        internal
        returns (LoadTestResult memory result)
    {
    ...
    function generateComparisonReport(
        ...
        if (normal.avgGasPerDeposit > 0) {
            uint256 gasIncreasePeak =
                ((peak.avgGasPerDeposit - normal.avgGasPerDeposit) * 100) / normal.avgGasPerDeposit;
            uint256 gasIncreaseStress =
                ((stress.avgGasPerDeposit - normal.avgGasPerDeposit) * 100) / normal.avgGasPerDeposit;
